...
pub struct SendMessageParams {
    ...
    pub baseline_local_id: Option<i64>,
}

#[cfg(test)]
mod tests {
    use super::{confirmation_can_finish, opened_chat_matches, selected_chat_matches, A11yNode};
    ...
    #[test]
    fn selected_chat_row_confirms_target_when_header_is_not_exposed() {
        let tree = A11yNode {
            role: "desktop-frame".into(),
            name: "main".into(),
            bounds: None,
            ...
            children: Some(vec![A11yNode {
                ...
                parent_index: None,
                window: None,
                states: Some(vec!["SELECTED".into()]),
                children: None,
            }]),
        };

        assert!(selected_chat_matches(&tree, "群测试"));
        assert!(!selected_chat_matches(&tree, "uncle篮球队"));
    }
}
...
fn selected_chat_matches(node: &A11yNode, chat_name: &str) -> bool {
    let expected = chat_name.trim();
    if expected.is_empty() {
        return false;
    }
    if node.role == "list-item"
        ...
        && node
            .states
            .as_ref()
            .is_some_and(|states| states.iter().any(|state| state == "SELECTED"))
    {
        return true;
    }
    node.children
        .as_ref()
        .map(|children| children.iter().any(|child| selected_chat_matches(child, expected)))
        .unwrap_or(false)
}
...
fn sent_message_is_newer(local_id: i64, baseline_local_id: Option<i64>) -> bool {
    baseline_local_id.map(|baseline| local_id > baseline).unwrap_or(true)
}

fn message_confirmed_in_db(params: &SendMessageParams, session_id: &str) -> bool {
    let message = match params.message.as_deref() {
    ...
    let account = match session.logged_in_user.as_deref() {
        Some(account) => account,
        None => return false,
    };
    let db = get_db();
    let keys = get_stored_keys(&db, &session.id, account);
    if !keys.keys().any(|name| name.starts_with("message_") && name.ends_with(".db") && !name.contains("fts") && !name.contains("resource")) {
        return false;
    }
    wechat_messages::list_messages(account, &keys, &params.chat_id, 20, 0)
        .into_iter()
        .any(|item| item.is_self == Some(true)
            && sent_message_is_newer(item.local_id, params.baseline_local_id)
            ...
            match &plan_state.phase {
                SendMessagePhase::Opening => {
                    ...
                    if main_state_id == Some("chat_open")
                        && params.chat_name.as_deref().is_some_and(|name| {
                            opened_chat_matches(a11y, name) || selected_chat_matches(a11y, name)
                        })
                    {
                ...
                SendMessagePhase::Focusing => {
                    ...
                    if let Some(chat_name) = params.chat_name.as_deref() {
                        if !opened_chat_matches(a11y, chat_name)
                            && !selected_chat_matches(a11y, chat_name)
                        {
                ...
                SendMessagePhase::Confirming => {
                    let message_visible = params
                        ...
                        .unwrap_or(false);
                    let message_in_db = message_confirmed_in_db(params, _session_id);
