...
fn require_group_chat_name(chat_id: &str, chat_name: Option<String>) -> Result<Option<String>, &'static str> {
    if chat_id.contains("@chatroom") && chat_name.as_deref().map(str::trim).unwrap_or("").is_empty() {
        return Err("CHAT_TARGET_UNRESOLVED");
    }
    Ok(chat_name)
}
...
#[cfg(test)]
mod tests {
    use super::require_group_chat_name;

    #[test]
    fn group_send_rejects_when_target_name_cannot_be_resolved() {
        assert_eq!(
            require_group_chat_name("53250352594@chatroom", None),
            Err("CHAT_TARGET_UNRESOLVED")
        );
    }
}
...
pub async fn send_message(Json(input): Json<SendParams>) -> Json<SendResult> {
    ...
    let logged_in_user = session.logged_in_user.clone().unwrap_or_default();
    let mut chat_keys = {
        let db = get_db();
        get_stored_keys(&db, &session.id, &logged_in_user)
    };

    // The chat list endpoint lazily extracts these keys. Sending must do the
    // same, otherwise a missing key silently leaves chat_name unset and the
    // UI automation can reuse whatever chat is currently open.
    if !chat_keys.contains_key("session.db") || !chat_keys.contains_key("contact.db") {
        let on_disk = list_account_dbs(&logged_in_user);
        let has_missing_directory_key = on_disk.iter().any(|name| {
            (name == "session.db" || name == "contact.db") && !chat_keys.contains_key(name.as_str())
        });
        if has_missing_directory_key {
            if let Some(pid) = find_wechat_pid() {
                let extracted = extract_keys_async(pid).await;
                if !extracted.is_empty() {
                    let db = get_db();
                    store_keys(&db, &session.id, &logged_in_user, &extracted);
                    chat_keys = get_stored_keys(&db, &session.id, &logged_in_user);
                }
            }
        }
    }

    let chat_name = wechat_chats::get_chat_by_username(&logged_in_user, &chat_keys, &input.chat_id)
        .map(|chat| chat.name);
    let chat_name = match require_group_chat_name(&input.chat_id, chat_name) {
        Ok(name) => name,
        Err(error) => {
            return Json(SendResult {
                success: false,
